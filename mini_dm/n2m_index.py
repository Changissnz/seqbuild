...
"""
generates a map of n-to-m relations between a vector 
space in dimension n and another vector space in dimension m. 
The map is generated by a pseudo-random generator `prg` and 
is essentially a sequence of tuples (nset,mset), such that 
`nset` and `mset` are stringized ordered sets of integers 
specifying their respective vector space indices. 

NOTE: 
generator is not guaranteed to produce an n-to-m 
index mapping such that the degree d_j (non-negative integer) 
of every j'th index in m-space satisfies `index_degree_range` 
(j'th value is associated with d_j different n-sets). The 
lack of guarantee is due to the lack of constraints on the 
integer generators `prg` and `prg2` used for decision-making, 
and are permitted to output relatively repetitive subsequences 
of values. 
"""
class N2MIndexMapGen: 
    ...
    def __init__(self,nm,index_degree_range,\
        ...
        self.n2m_imap = N2MIndexMap(self.nm,set()) 
